...
export const listMonitorGroups = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<MonitorGroupDTO[]> => {
    ...
    // Seleção existente (mantém marcados mesmo após atualizar).
    const { data: monitored } = await (supabase as any)
    ...
    for (const m of monitored ?? []) {
      ...
      // Se o grupo salvo não aparece mais na conta, ainda assim mantém no card
      // como "unavailable" para o usuário poder desmarcar.
